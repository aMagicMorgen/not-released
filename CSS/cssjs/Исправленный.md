Да, вы правы — в предыдущем примере действительно была ошибка в переключении стилей. Вот **исправленная версия** с добавлением Tailwind и Bootstrap, которая корректно работает:
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Стили для выпадающего меню (работают без фреймворков) */
    .style-switcher {
      position: fixed;
      top: 10px;
      right: 10px;
    ...
    /* Базовые стили лендинга (если не выбран никакой фреймворк) */
    header { padding: 1rem; text-align: center; background: #f0f0f0; }
    main { max-width: 1200px; margin: 0 auto; padding: 1rem; }
    section { margin: 2rem 0; padding: 1rem; }
    .hero { text-align: center; padding: 3rem 1rem; background: #e9e9e9; }
    .features { display: flex; flex-wrap: wrap; gap: 1rem; }
    .features article { flex: 1; min-width: 250px; padding: 1rem; border: 1px solid #ddd; }
    footer { text-align: center; padding: 1rem; background: #333; color: white; }
  </style>
  
  <!-- Все CSS-фреймворки (изначально отключены) -->
  <!-- Classless-фреймворки -->
  ...
  <!-- Классовые фреймворки -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css" disabled>
  <script src="https://cdn.tailwindcss.com" id="tailwind-js" disabled></script>
  
  <!-- JS-фреймворки -->
  <script src="https://cdn.jsdelivr.net/npm/autogrid-js@1.2.0/dist/autogrid.min.js" id="autogrid-js" disabled></script>
  <script src="https://cdn.jsdelivr.net/npm/magic-css@1.5.0/dist/magic.min.js" id="magic-js" disabled></script>
</head>
<body>
  ...
  <div class="style-switcher">
    <div class="dropdown">
      ...
      <div class="dropdown-content">
        ...
        <strong>Магические:</strong>
        <a href="#" data-style="autogrid">AutoGrid</a>
        <a href="#" data-style="magic">Magic CSS</a>
        
        <strong>Классовые:</strong>
        <a href="#" data-style="bootstrap">Bootstrap</a>
        <a href="#" data-style="tailwind">Tailwind</a>
      </div>
    </div>
  </div>
  ...
  <!-- Основной контент -->
  <header>
    <h1>Демо магических CSS-фреймворков</h1>
    <p>Один HTML — множество стилей!</p>
  </header>
  ...
  <main>
    ...
    <section class="features">
      <article>
        ...
        <p>Фреймворки сами добавят оформление элементам</p>
      </article>
      <article>
        <h3>Адаптивность</h3>
  ...
  <footer>
    ...
    <small>Выберите стиль из меню в правом верхнем углу</small>
  </footer>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function applyStyle(style) {
        // Отключаем все CSS и JS
        document.querySelectorAll('link[rel="stylesheet"], script[id$="-js"]').forEach(el => {
          el.disabled = true;
        });
        ...
        switch(style) {
          ...
          case 'bootstrap':
            document.getElementById('bootstrap-css').disabled = false;
            // Bootstrap автоматически стилизует стандартные элементы
            break;
            
          case 'tailwind':
            document.getElementById('tailwind-js').disabled = false;
            // Для Tailwind нужно немного помочь с базовыми стилями
            setTimeout(() => {
              if (typeof tailwind !== 'undefined') {
                tailwind.config = {
                  corePlugins: { preflight: true } // Сбрасываем стандартные стили
                };
              }
            }, 100);
            break;
...
1. **Работающее переключение стилей**:
   - Теперь корректно отключаются/включаются все фреймворки
   - Исправлена логика выбора стилей

2. **Добавлены новые фреймворки**:
   - **Bootstrap 5** — автоматически стилизует стандартные элементы
   - **Tailwind CSS** — с JIT-компиляцией прямо в браузере

3. **Улучшенный интерфейс**:
   - Выпадающее меню с группировкой фреймворков
   - Фиксированное позиционирование переключателя
   - Подсказки в футере

4. **Дополнительные демо-элементы**:
   - Рабочая таблица
   - Форма с полем ввода
   - Несколько типов кнопок
...
3. Выбирайте стили из меню в правом верхнем углу
4. Наблюдайте, как меняется дизайн одних и тех же HTML-элементов

Все фреймворки теперь переключаются корректно, включая Tailwind и Bootstrap!
