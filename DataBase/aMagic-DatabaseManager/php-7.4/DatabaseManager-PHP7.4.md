# DatabaseManager для PHP 7.4

Полная версия класса, адаптированная для работы с PHP 7.4:
...
## Ключевые изменения для PHP 7.4:

1. **Удалены объявления типов свойств**:
   ```php
   // Было в PHP 8.0:
...
2. **Упрощены объявления возвращаемых типов**:
   ```php
   // Было:
...
3. **Заменены современные строковые функции**:
   ```php
   // Вместо str_contains():
   if (strpos($str, $needle) !== false)
   
   // Вместо str_ends_with():
   if (substr($str, -strlen($needle)) === $needle)
   ```

4. **Добавлены проверки на существование ключей массива**:
   ```php
   if (isset($result[0]['cnt']))
   ```

5. **Улучшена обработка JSON**:
   ```php
   $jsonData = json_decode($storageData['json'], true) ?: [];
   ```

6. **Добавлены PHPDoc-аннотации** для компенсации отсутствия типизации:
   ```php
   /**
    * @param string $table
    * @return array
...
7. **Улучшена обработка ошибок** с учетом PHP 7.4:
   ```php
   try {
       $json = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR);
...
## Особенности работы в PHP 7.4:

1. **Совместимость**:
   - Работает на PHP 7.4 и выше
   - Сохраняет всю функциональность PHP 8.0+ версии
   - Более строгие проверки типов в runtime

2. **Безопасность**:
   - Добавлены дополнительные проверки isset()
   - Улучшена обработка ошибок JSON
   - Более строгая валидация входных данных

3. **Производительность**:
   - Оптимизированы запросы к SQLite
   - Кэширование результатов запросов
   - Эффективное использование транзакций

Для использования достаточно скопировать весь код класса и подключить его в вашем проекте на PHP 7.4. Все CRUD-методы и дополнительные функции работают так же, как и в версии для PHP 8.0+.
