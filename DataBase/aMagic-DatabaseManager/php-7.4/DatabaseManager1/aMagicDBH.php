...
class MySQLDriver extends DatabaseDriver {
    ...
    public function formatDate(string $func): string {
		...
		return $functions[strtoupper($func)] ?? "DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i:%s')";
...
class SQLiteDriver extends DatabaseDriver {
    ...
    public function formatDate(string $func): string {
		...
		$format = $formatMap[strtoupper($func)] ?? '%Y-%m-%d %H:%M:%S';
		/* //PHP 8.0
...
// ДОБАВЛЕНО
/*
//abstract protected static function validateConfig(array $config): void{
public static function validateConfig(array $config): void {
    ...
    // Дополнительные проверки в зависимости от драйвера
    ...
    if ($driver === 'sqlite') {
        ...
        if (!isset($config['host'])) {
            throw new InvalidArgumentException("MySQL requires 'host' parameter in config");
        }
    }
}
*/
public static function validateConfig(array $config): void {
    // Для SQLite проверяем наличие path вместо dbname
    if (($config['driver'] ?? null) === 'sqlite') {
        if (!isset($config['path'])) {
            throw new InvalidArgumentException("SQLite requires 'path' parameter in config");
        }
    } 
    // Для MySQL и других СУБД проверяем dbname
    ...
    $result = self::select($table, $conditions, $columns, 1); // limit передаётся как 4-й аргумент
...
*/
    ...
    self::$driver = new $drivers[$driverName]();
}
