...
public static function createDocument(array $data): string {
    ...
    try {
        ...
        // Получаем список полей таблицы через PRAGMA
        ...
        #error_log("Returning docId: " . var_export($docId, true) . ", type: " . gettype($docId));
        return $docId;
    } catch (Exception $e) {
...
private static function getTableColumns(string $table): array {
    ...
    // Используем прямой SQL-запрос через execute
...
public static function updateDocument(int $id, array $data): int {
    $json = json_encode($data, JSON_UNESCAPED_UNICODE);
    if ($json === false) {
        throw new RuntimeException("Failed to encode document data");
    }
    ...
    try {
        ...
        $tableFields = self::getTableColumns(self::$mainTable);
        $fieldsToUpdate = array_intersect_key($data, array_flip($tableFields));

        // Обновляем основную таблицу
        $rowsAffected = parent::update(
            self::$mainTable, 
            $fieldsToUpdate, 
            ['id' => $id]
        );
...
public static function deleteDocument(int $id): int {
    ...
    try {
        ...
        $storageRows = parent::delete(
            self::$storageTable,
            ['doc_id' => $id]
        );

        // 2. Удаляем из основной таблицы
        $mainRows = parent::delete(
            self::$mainTable,
            ['id' => $id]
        );
        ...
        return $mainRows; // Возвращаем количество удаленных строк из основной таблицы
    } catch (Exception $e) {
        if ($transactionStarted && parent::inTransaction()) {
            parent::rollBack();
        }
