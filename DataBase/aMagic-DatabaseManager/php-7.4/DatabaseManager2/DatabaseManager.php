...
// В класс aMagicDBH добавляем:
protected static function getDriver(): DatabaseDriver {
    if (!self::$driver) {
        throw new RuntimeException("Database driver not initialized");
    }
    return self::$driver;
}
...
public static function createDocument(array $data): int {
    ...
    $shouldCommit = !parent::inTransaction();
    ...
    try {
        ...
        if ($shouldCommit && parent::inTransaction()) {
            parent::rollBack();
        }
        throw $e;
    }
...
public static function createDocument(array $data): int {
    ...
    if (empty($data)) {
        throw new InvalidArgumentException("Document data cannot be empty");
    }
    ...
    $text = implode(' ', array_values($data)); // Более осмысленное объединение
    ...
    try {
        ...
        $tableColumns = self::getTableColumns(self::$mainTable);
        $filteredData = array_intersect_key($data, array_flip($tableColumns));
        
        // Убедимся, что есть данные для вставки
        if (empty($filteredData)) {
            $filteredData = ['created_at' => date('Y-m-d H:i:s')];
        }

        $docId = self::insert(self::$mainTable, $filteredData);
...
/*
    ...
    $transactionStarted = false;
    try {
        // Начинаем транзакцию только если её ещё нет
        if (!parent::inTransaction()) {
            parent::beginTransaction();
            $transactionStarted = true;
        }
...
public static function createDocument(array $data): int {
    ...
    // Используем parent:: для вызова методов транзакции
    parent::beginTransaction();
...
protected static function getTableColumns(string $table): array {
    ...
    if (self::$driver instanceof SQLiteDriver) {
        $result = self::selectRaw("PRAGMA table_info({$table})");
        ...
        // Для MySQL/других СУБД
        $result = self::selectRaw("SHOW COLUMNS FROM {$table}");
        foreach ($result as $row) {
            if (isset($row['Field'])) {
                $columns[] = $row['Field'];
            }
...
protected static function getTableColumns(string $table): array {
    $columns = [];
    $driver = parent::getDriver(); // Получаем драйвер через родительский метод
    
    if ($driver instanceof SQLiteDriver) {
        $result = self::selectRaw("PRAGMA table_info(" . self::quoteIdentifier($table) . ")");
        foreach ($result as $row) {
            if (isset($row['name'])) {
                $columns[] = $row['name'];
            }
        ...
        // Для MySQL/других СУБД
        $result = self::selectRaw("SHOW COLUMNS FROM " . self::quoteIdentifier($table));
        foreach ($result as $row) {
            if (isset($row['Field'])) {
                $columns[] = $row['Field'];
            }
...
protected static function selectRaw(string $sql, array $params = []): array {
    try {
        $stmt = parent::getConnection()->prepare($sql);
        $stmt->execute(parent::normalizeParams($params));
