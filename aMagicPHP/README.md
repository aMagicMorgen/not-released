# not-released/aMagicPHP
## ПОКА В РАЗРАБОТКЕ НО РАБОТАЕТ
...
### Пример config.php 
тоже реализован так, что все настройки по выводу уже в нем.
...
```
### Если а `$sections = 'section01';` добавить еще названия секций `через запятую` в строку или в столбик `через перенос строки`
будут созданы эти блоки с хэталонными записями как файл `section01.php`
